<script>
  ...
  onMount(async () => {
    try {
      ...
      subscriptionKind1.on('event', async (event) => {
        const reactedEventId = event.tags.find(tag => tag[0] === 'e')[1];

        // Fetch the original event (kind 1)
        const originalEvent = await ndk.fetchEvent({ id: reactedEventId, kinds: [1] });

        const originalContent = originalEvent ? originalEvent.content : 'Original event not found.';
        ...
        notifications = [...notifications, {
          id: event.id,
          pubkey: event.pubkey,
          eventReactedTo: reactedEventId,
          originalContent // Save the content of the original event
        }];
        isLoading = false;
      });
      ...
      <div class="notification-card">
        ...
        <div class="profile-card">
          ...
          <div>
            ...
            <!-- Display the original content instead of the event ID -->
            <p>{notification.originalContent}</p>
          </div>
        </div>
      </div>
    {/if}
